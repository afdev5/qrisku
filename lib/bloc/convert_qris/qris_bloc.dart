    ...
    on<QrisEvent>((event, emit) async {
      ...
      if(event is GetAllQris) {
        try {
          ...
          emit(state.copyWith(datas: event.isFromHome ? datas.take(3).toList() : datas, isLoading: false)); // jika dari home screen hanya ambil 3 data
        } catch(e) {
        ...
        var qrData = await QrisService().convertQris(qris: qrDefault!, nominal: event.nominal, fee: event.fee, taxIsPercent: event.taxIsPercent);
        await service.insertData(ConvertQrisModel(
            dataQr: qrData,
            nominal: event.fee != null ? (double.parse(event.nominal) + double.parse(event.fee!.isNotEmpty ? event.fee! : '0')).toString() : event.nominal,
            dateTime: DateTime.now(),
            title: ''
        ));
        var datas = await service.getDatas();
        emit(state.copyWith(datas: datas, isLoading: false, message: 'Berhasil;@$qrData'));
        try {
